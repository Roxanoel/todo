...
function handleTickboxClicked(e) {
  const index = e.path.find((item) => item.classList.contains("card")).dataset
    .index;
...
function handleEditTaskClicked(e) {
  ...
  const titleContent = document.querySelector(
    `.card[data-index="${index}"] .task-title`
  ).textContent;
  const dateContent = document
    .querySelector(`.card[data-index="${index}"] .date`)
    .textContent.slice(4); // Removes the "Due: ", for now. There is probably a better way to do this.
  const descrContent = document.querySelector(
    `.card[data-index="${index}"] .descr`
  ).textContent;
